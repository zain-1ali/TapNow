...
const ContactModal = ({contactModal,handlecontactModal,contactDetails,deleteModal,handledeleteModal}) => {
...
let deleteContact=()=>{
    remove(ref(db , `User/${contactDetails?.connectedWith?.id}/contactRequests/${contactDetails?.id}` )).then(()=>{
        // handleModal()
  ...
  return (
    <Modal
        open={contactModal}
        onClose={() =>{handlecontactModal(),handledeleteModal()} }
...
<div className='w-[97%] flex justify-end  mt-[10px]'> 
    <RxCross2 className="text-black cursor-pointer text-xl" onClick={() =>{handlecontactModal(),handledeleteModal()}}/>
    </div>
...
<p className='font-medium text-sm w-[90%]'>{contactDetails?.message}</p>
</div>
    </div>
