...
<div className='w-[97%] flex justify-end  mt-[10px]'> 
    <RxCross2 className="text-black cursor-pointer text-xl" onClick={() =>handleModal()}/>
    </div>
...
<p>{userdata?.Analytics?.totalClicks ? userdata?.Analytics?.totalClicks :0}</p>
</div>

<div className='w-[50%] h-[100%] flex  items-center justify-center'>
<img src={userdata?.profileUrl ? userdata?.profileUrl :"https://placehold.co/90x90"} alt="profile" className='h-[90px] w-[90px] rounded-full shadow-md'/>
</div>
...
<p>{userdata?.Analytics?.linksEngCrntWk ? userdata?.Analytics?.linksEngCrntWk :0}</p>
</div>


<div className='flex w-[85%] justify-between mt-2'>
<h2 className='text-sm font-medium'>Since last week</h2>
<p>{userdata?.Analytics?.linksEngPstWk ? userdata?.Analytics?.linksEngPstWk :0}</p>
</div>
...
<div className='flex w-[85%] justify-between mt-2'>
<h2 className='text-sm font-medium'>Since last week</h2>
<p>{userdata?.Analytics?.tContactsMePstWk ? userdata?.Analytics?.tContactsMePstWk :0}</p>
</div>
</div>
...
<div className='w-[90%] h-[160px] mt-3 p-2 overflow-y-scroll scrollbar-hide'>
    {Object.values(userdata?.Analytics?.links)?.map((elm)=>{
        return <div className='w-[100%] h-[50px] bg-white rounded-md shadow-md mt-2 p-3 flex justify-between'>
        <div className='w-[100%] flex justify-between items-center '>
        <div className='flex'>
            <img src={returnIcons(elm?.name)} alt="" className='h-[30px] w-[30px]'/>
            <h2 className='text-sm font-medium ml-2'>{elm?.name}</h2>
        </div>
        </div>
